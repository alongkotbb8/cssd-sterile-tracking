...
> การพิมพ์มี 2 โหมดที่อนุมัติ: **`PRINT_GATEWAY`** (Print Gateway → Linux/CUPS → Xprinter XP-420B `usb_spool`, hardware ACK — เส้นทางหลัก) และ
> **`BROWSER_DIALOG`** (macOS print dialog บน Mac ที่เสียบ XP-420B + เปิด PWA เครื่องเดียวกัน, หลัง flag `CSSD_BROWSER_PRINT_ENABLED` default off,
> ผู้ใช้ยืนยันผลเอง `USER_CONFIRMED` — **ไม่ใช่** direct USB/WebUSB); **ยกเลิก offline/Bluetooth Pilot/A318BT/Zebra/mobile-first**.
...
11. **ทุกครั้งที่พิมพ์ label ต้องบันทึก** `printedAt`/`reprintCount` + AuditLog — **ทางเดียว** คือ Print Gateway เรียก `POST /print-gateway/jobs/:id/ack` **ห้าม** client (PWA/มือถือ) ตั้งค่านี้เองเด็ดขาด (เปิด print dialog สำเร็จ ≠ พิมพ์สำเร็จจริง)
    - โหมด `BROWSER_DIALOG` เก็บประวัติแยกใน `BrowserPrintRequest` (`CREATED → DIALOG_OPENED → USER_CONFIRMED|CANCELLED`; `USER_CONFIRMED` = ผู้ใช้ยืนยันเอง ไม่ใช่ hardware) — **ห้ามแตะ** `printedAt`/`reprintCount` และห้ามใช้สถานะ `PRINTED/SENT/ACK_UNKNOWN` กับโหมดนี้; reprint นับประวัติรวมทั้งสองโหมด + บังคับ `reprintReason`
...
- **Browser Print (`BROWSER_DIALOG`)** — โหมดเสริม (MACOS_BROWSER_PRINT_DIRECTIVE.md):
  PWA บน Mac ที่เสียบ XP-420B เปิด macOS print dialog (PDF ขนาด label จริง); backend เก็บ
  `BrowserPrintRequest` (API `browser-print-requests`) แยกจาก PrintJob; ผู้ใช้ยืนยันผลเอง;
  เปิดด้วย `CSSD_BROWSER_PRINT_ENABLED=true` ทั้ง backend + PWA build (default ปิด);
  iPhone/iPad/Android ควบคุม USB printer บน Mac ผ่านโหมดนี้ไม่ได้ — ต้องใช้ Print Gateway
