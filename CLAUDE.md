...
- **Mobile/PWA:** Flutter (Dart) — cross-platform เดียวจบ (Android/iOS + web/Chrome PWA)
  ...
  - Offline storage: `drift` (SQLite) + sync queue — **ยังไม่ implement** (ดูข้อ 6)
  - Bluetooth printer ต่อตรง: `flutter_blue_plus` + `print_bluetooth_thermal` (fallback เมื่อไม่มี Gateway)
...
- เครื่องพิมพ์ label เป้าหมาย = **Xprinter XP-420B** (USB, 203 DPI, TSPL/TSPL2) — renderer TSPL
  bitmap เข้ากันได้ แต่ USB มักเป็น **printer-class ไม่ใช่ COM/serial** → ต้องเลือก transport ตาม
  host OS (Windows spooler / Linux `/dev/usb/lp0` / mac CUPS) ก่อน FIX-08 (ดู `HARDWARE_VERIFICATION.md`
  ข้อ 0.1); dev ใช้ console/mock transport
