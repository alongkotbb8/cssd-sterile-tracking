...
export class MasterDataService {
  ...
  async createTag(name: string, colorHex: string | undefined, userId: string) {
    ...
    if (existing) throw new ConflictException({ message: 'มี tag ชื่อนี้อยู่แล้ว', code: 'TAG_DUPLICATE' });
  ...
  async createTemplate(dto: CreateSetTemplateDto, userId: string) {
    ...
    if (existing) throw new ConflictException({ message: 'รหัสชุดอุปกรณ์นี้มีอยู่แล้ว', code: 'TEMPLATE_DUPLICATE' });
