...
|---|---|---|
| 2026-07-24 | **Dashboard "ชุดอะไรไปอยู่ที่ไหน" + ค้นหา + ลบหลายรายการ (field-test tooling):** **(1) ค้นหา** — `GET /packages?search=` ค้น (case-insensitive contains) เลขห่อ / ชื่อชุด / **ชื่ออุปกรณ์ในชุด** (itemList) / **ชื่อคลัง-แผนกปัจจุบัน** (ผ่าน movement) + ต่อช่องค้นหาเดิมในหน้ารายการเข้ากับ server search. **(2) Dashboard** — section "ชุดอะไรไปอยู่ที่ไหน (ล่าสุด)" จาก `dashboard().recentMovements` (8 ล่าสุด: ชุด/เลขห่อ/ชนิด IN·OUT·RETURN/แผนก/เวลา → แตะเข้า detail) + ช่องค้นหาบนหน้าแรก (debounce → ผลลัพธ์ inline → แตะเข้า detail). **(3) ลบหลายรายการ** — `POST /packages/bulk-delete` (SUPERVISOR/ADMIN, Idempotency-Key, per-item result) **ลบถาวรเฉพาะห่อ PACKED ที่ไม่มีประวัติ** (batchId/printedAt/reprintCount ว่าง + ไม่มี movement/printJob/browserPrintRequest/batchAttempt) มิฉะนั้นปฏิเสธ `PKG_HAS_HISTORY` (รักษา traceability) + audit `PACKAGE_DELETE` ก่อนลบ; UI = ปุ่ม "ลบถาวร" คู่ปุ่มพิมพ์ในแถบเลือกหลายรายการ (โผล่เฉพาะ filter=PACKED + สิทธิ์สูง) + **popup ยืนยันก่อนลบจริง**. **Verify:** api build + **215/215** unit (+search/bulk-delete/rbac), **integration 41/41** บน PG16 จริง (bulk-delete PACKED-only/no-history/RBAC + search), flutter analyze 0 + **198/198** (+dash_search_delete_test), gateway 33/33, npm audit high+ = 0, web build ✓ | `apps/api/src/modules/packages/{packages.controller,packages.service,dto/bulk-delete.dto}.ts` + `__tests__/{packages.service,packages.rbac}.spec.ts`, `reports.service.ts` + `reports/__tests__`, `test/integration/packages.int-spec.ts`, `packages/shared/error-codes.json` (PKG_HAS_HISTORY), mobile `dashboard_page.dart`/`packages_page.dart`/`repositories.dart`/`models.dart`/`api_client.dart`/`l10n/*.arb` + `test/dash_search_delete_test.dart` |
| 2026-07-24 | **แก้ 2 blockers Browser Print รอบ QA (P0 patient-safety + P1 concurrency):** **P0 — label แสดงวันที่เก่าหลัง reprocess:** เดิม `isSterilized` ตัดสินจากการมี `sterilizeDate` เท่านั้น → ห่อที่ reprocess กลับเป็น PACKED แต่ยังมีวันที่รอบเก่าค้าง อาจพิมพ์วันที่ผิด. แก้ 2 ชั้น: (1) label gate ด้วย **`status===STERILE && มีวันที่ครบ`** เท่านั้น มิฉะนั้นส่ง `null` + `isSterilized=false`; (2) **root cause** — `scanReprocess` (RETURNED→PACKED) ล้าง `sterilizeDate/expiryDate` ใน DB (แก้ทั้ง gateway label/detail/FEFO ด้วย; ประวัติ `PackageBatchAttempt` ไม่ถูกแตะ). **P1 — การตัดสิน reprint ไม่ atomic:** สองคำขอ key คนละค่ายิงพร้อมกันอาจอ่านว่ายังไม่เคยพิมพ์ทั้งคู่ → `isReprint=false` ทั้งคู่. แก้: **`pg_advisory_xact_lock` ต่อ packageId** ใน tx สร้างคำขอ (serialize) + ขยายนิยาม isReprint ให้นับ prior ที่ **ยังไม่ถูกยกเลิก** (CREATED/DIALOG_OPENED/USER_CONFIRMED) หรือ gateway printedAt → คำขอที่สองถูกบังคับเป็น reprint + ต้องมีเหตุผล (CANCELLED ไม่นับ). **Tests:** +4 unit (P0 label PACKED/RETURNED stale-date, P1 prior-CREATED reprint, CANCELLED-ไม่นับ) = api **193/193**; +4 **PostgreSQL integration** (P1 `Promise.allSettled` ยิงจริง 2 คำขอ key คนละค่า → 1 success isReprint=false + 1 ถูกบังคับ reprint; P1 sequential; P0 reprocess ล้างวันที่จริงใน DB; P0 STERILE ไม่ over-block) = **31/31**; e2e browser-print **40/40** ทั้ง 4 projects (verify กับ stack จริงหลัง rebuild API); gateway 33/33; npm audit high+ = 0 | `apps/api/src/modules/browser-print/browser-print.service.ts` (label gate + advisory lock + isReprint), `apps/api/src/modules/scan/scan.service.ts` (reprocess clears dates), `apps/api/test/integration/{harness,browser-print.int-spec}.ts`, `browser-print.service.spec.ts` |
| 2026-07-24 | **Browser Print mode (`BROWSER_DIALOG`) ตาม MACOS_BROWSER_PRINT_DIRECTIVE.md — code-complete:** โหมดพิมพ์ผ่าน macOS system print dialog บน Mac ที่เสียบ XP-420B (**คง Print Gateway เดิมครบถ้วน** — ไม่มีไฟล์ gateway ถูกแตะ; ไม่ใช่ WebUSB). **Backend**: ตาราง `BrowserPrintRequest` + 3 enums (migration additive/reversible — **พิสูจน์บน PG16 จริง 3 แบบ: fresh / existing-data(seed แล้ว migrate ทับ) / down→re-apply**), module ใหม่ 6 endpoints (`/browser-print-requests` create/dialog-opened/confirm/cancel/list/get) — flag `CSSD_BROWSER_PRINT_ENABLED` (default **ปิด**, ค่าเพี้ยน = ไม่ยอมบูต, ตรวจทุก endpoint), state machine `CREATED→DIALOG_OPENED→USER_CONFIRMED\|CANCELLED` (CAS pin สถานะที่อ่านจริง — แก้ race ที่ audit previousStatus ผิดจาก adversarial review), isReprint คำนวณ backend (ประวัติ browser DIALOG_OPENED/USER_CONFIRMED + gateway printedAt) + บังคับเหตุผล + `prior` ใน error body, IDOR (เจ้าของ/SUPERVISOR/ADMIN อ่าน; เจ้าของเท่านั้น transition), Idempotency-Key required ทุก mutation, per-user rate limit (`BROWSER_PRINT_THROTTLE_MAX`), audit 5 actions ใน tx เดียว, **ไม่แตะ `printedAt`/`reprintCount`/PrintJob เด็ดขาด**; error codes ใหม่ 6 ตัวเข้า registry+ARB th/en. **Mobile**: sheet ตาม §10 (reprint gate→create ด้วย key คงเดิม→preview จาก label payload ฝั่ง backend เท่านั้น (bitmap เดิมของ label_renderer; ห่อไม่ sterile = แถบเตือน ไม่มีวันที่)→dialog-opened ต้องสำเร็จก่อน→PDF 60×40มม.×copies→print seam (เพิกเฉยผลลัพธ์ 100%)→ผู้ใช้เลือกผลเอง 3 ทาง), ปุ่ม+history ซ่อนหลัง flag ทุกจุด (detail/create-success/print-jobs segmented), §12 wording ตรงเป๊ะ. **Tests**: api **189/189** (+54; §14 backend ครบ 1–17 + flag/throttle/race), flutter analyze 0 + **197/197** (+16 §14 Flutter ครบ 1–13 (+24 layout matrix), web build ✓, e2e spec ใหม่ 10 เคส (§14 E2E ครบ 1–9, typecheck ✓) + CI/stack เปิด flag เฉพาะ test stack, gateway **33/33** ไม่เปลี่ยน, integration **27/27** บน PG16 จริง, npm audit high+ = 0, osv-scanner = no issues. **Docs §16 ครบ**: CLAUDE/AGENTS/GUARDRAILS (§2.1 ใหม่)/master directive (§13 addendum)/README/e2e README + ใหม่ `docs/MAC_XP420B_BROWSER_PRINT.md` (คู่มือ+hardware acceptance 16 ข้อ) + `docs/UAT_CHECKLIST.md`. **สถานะตาม §18: สูงสุด CI_VERIFIED — HARDWARE: NOT VERIFIED (ต้องทดสอบ Mac+XP-420B จริงตาม §15), PILOT/PRODUCTION: NOT APPROVED** | `apps/api/prisma/schema.prisma`+migration, `apps/api/src/modules/browser-print/**` (ใหม่), `app.module.ts`, `packages/shared/error-codes.json`, `apps/mobile/lib/features/browser_print/**` (ใหม่), `feature_flags.dart`, `models.dart`, `repositories.dart`, `api_client.dart`, `package_detail_page.dart`, `create_package_sheet.dart`, `print_jobs_page.dart`, `l10n/*.arb`, `test/{browser_print,layout}_test.dart`, `e2e/tests/browser-print.spec.ts` (ใหม่), `.github/workflows/ci.yml`, `scripts/e2e-stack.sh`, docs ตาม §16 |
| 2026-07-24 | **Gate 3 blocker แก้ + 3 ข้อสังเกตที่ QA ไม่บล็อกแต่ให้ติดตาม (ที่ commit `d7ea4fd`):** **(A) Branch protection main** — สร้าง GitHub Ruleset (`id 19646615`, https://github.com/alongkotbb8/cssd-sterile-tracking/rules/19646615): target `refs/heads/main`, enforcement active, **`bypass_actors: []`** (`current_user_can_bypass: never` — เข้มกว่า classic branch protection ที่ admin bypass ได้เสมอ), rule `pull_request` (ต้องผ่าน PR; `required_approving_review_count: 0` — **ตัดสินใจไม่บังคับ approve เพราะ repo มีผู้ดูแลคนเดียว, GitHub ไม่ให้ approve PR ตัวเอง การบังคับ 1 approval จะ deadlock การ merge ของเจ้าของ repo เอง**, ยังเปิด `dismiss_stale_reviews_on_push: true` ไว้), rule `required_status_checks` (8 ตัวตรงกับ job name จริงบน CI: api-gateway/integration/mobile/e2e/secret-scan/npm-audit/flutter-audit/sbom, `strict_required_status_checks_policy: true` = ต้อง up-to-date กับ main), rule `deletion` + `non_fast_forward` (กันลบ/force-push). **(B) osv-scanner ปักหมุด SHA256** ใน `security-scan.yml` (เทียบกับ `osv-scanner_SHA256SUMS` ที่ google เผยแพร่คู่ release — กัน supply-chain tampering ของตัว release asset เอง ไม่ใช่แค่ pin เวอร์ชัน). **(C) `scripts/security-scan.sh` parity กับ CI** — เพิ่ม osv-scanner (ใช้ binary บน PATH ถ้ามี ไม่งั้นดาวน์โหลด+verify SHA256 ตาม platform แล้วรัน `scan --lockfile=pubspec.lock`, blocking เหมือน CI). **(D) 16 moderate ที่เหลือ — บันทึกเป็น tracked debt ไม่ใช่ fix เงียบ ๆ:** `@nestjs/core`/`body-parser` (ต้อง NestJS **v10→v11 ทั้ง stack** — breaking กว้างเกินจะรีบทำ, ต้องมี regression รอบใหญ่แยกต่างหาก), `uuid` (fix จริงต้อง `^11.1.1` แต่ `exceljs` ล็อก `^8.3.0` — override ข้าม range เสี่ยง runtime break ที่ไม่มี test cover), `file-type` (fix ต้อง `22.x` แต่ `@nestjs/common@10.4.22` ล็อกตรง `20.4.1` เป๊ะ — เช่นกัน). **Owner: ผู้ดูแล repo (alongkotbb8). Review trigger: ก่อนเริ่ม NestJS v11 migration ในอนาคต หรือ dependency review รอบถัดไป** — ทั้งหมดเป็น moderate ต่ำกว่าเกณฑ์ block ของ Gate 3 อยู่แล้ว | GitHub Ruleset (repo settings, ไม่มีไฟล์ในโค้ด), `.github/workflows/security-scan.yml` (SHA256 verify), `scripts/security-scan.sh` (osv-scanner parity) |
| 2026-07-23 | **แก้ 6 blockers รอบ QA (CI/Security แดงที่ `08d34ce`):** **(1) npm audit full-tree = 0 high+** (เดิม 1 critical + 4 high, แล้ว advisory ใหม่โผล่ระหว่างแก้: js-yaml/lodash/multer/libxmljs2): `bcrypt ^5.1.1→^6.0.0` (ทิ้ง node-pre-gyp/tar@6 subtree ทั้งหมด, hash `$2b$` เข้ากันได้), `@nestjs/cli ^10→^11.0.24` (webpack 5.106.2 patched), overrides pin เวอร์ชันที่ patch แล้ว: tar 7.5.21 / qs 6.15.3 / lodash 4.18.1 / multer 2.2.0 / js-yaml@4 4.3.0 (คง js-yaml 3.x ของ ts-jest) / libxmljs2 0.37.0. **ต้นเหตุ override ไม่ติดก่อนหน้า = stale `node_modules` (npm คง resolution เดิม) → ต้อง `rm -rf node_modules` ตอน regen**. เหลือ 16 moderate (uuid ใน firebase-admin/google-cloud ยังไม่มี fix non-breaking; ต่ำกว่าเกณฑ์ block). blocking gate = full `npm audit --audit-level=high`. **(2) readiness ยิงผิด URL** → `/api/v1/health` (เดิม `/api/v1` = 404 route, curl -f fail แม้ API พร้อม) ทั้ง `ci.yml` + `e2e-stack.sh`. **(3) `e2e-stack.sh` fail-fast** ครบ PG/API/PWA (exit 1 เมื่อครบ timeout; PWA เดิมไม่มี probe เลย). **(4) Flutter CVE scan จริง** = osv-scanner (OSV DB) pin v2.4.0 สแกน `pubspec.lock` แบบ blocking (เดิมมีแค่ pub outdated). **(5) e2e boot** (จากรอบก่อน) `NODE_ENV=e2e`. **A3 regression (node 20 = CI parity):** npm ci ✓ · audit high+ exit 0 ✓ · api build+**135/135** · gateway **33/33** · SBOM 330 · flutter analyze 0+**157/157**+web build ✓ · osv pubspec.lock = no issues | `.github/workflows/{ci,security-scan}.yml`, `scripts/e2e-stack.sh`, `apps/api/package.json` (bcrypt@6/cli@11), `package.json` (overrides), `package-lock.json` |
| 2026-07-23 | **Post-Gate-1 technical hardening (master directive ส่วน A2) — ยังไม่ push (รออนุญาต):** **(A2.1) error code ครบทุก user-facing exception** ที่เหลือ: `package-id.util`(PKG_ID_INVALID), `running-number`(RUNNING_NUMBER_FAILED), gateway auth guard(GATEWAY_KEY_*), master-data(TAG/TEMPLATE_DUPLICATE), departments(DEPT_DUPLICATE), reports(CLEANUP_DATE_INVALID), idempotency(IDEMPOTENCY_CONFLICT), jwt/auth(AUTH_SESSION_REVOKED/USER_NOT_FOUND), login throttle(AUTH_RATE_LIMITED). **(A2.2) ทะเบียนกลางข้ามภาษา** `packages/shared/error-codes.json` (client/internal) + jest backend (code ที่ throw ⊆ ทะเบียน + ไม่มี client code ค้าง) + Flutter `error_code_mapping_test` (client code ทุกตัวถูก map th+en). **(A2.3)** unknown error บน en ไม่มีอักษรไทย (regex ตรวจ). **(A2.4-7) validate env** `LOGIN_THROTTLE_MAX/WINDOW_MS`: pure `resolveThrottleConfig` (จำนวนเต็ม>0, เพดาน prod, ขอบเขตสัมบูรณ์, override ผ่อนปรนเฉพาะ test/e2e/ci, **fail-fast prod config ผิด**) + unit tests valid/invalid/boundary. **(A2.8) CI health-check fail-fast** (ครบ timeout ยังไม่พร้อม→exit 1). **(A2.9-10) pin `http-server`+`@cyclonedx/cyclonedx-npm` ใน root lockfile** เลิก `npx --yes` ใน CI/stack. **(A2.11)** CI รัน 4 projects: chromium+mobile-chrome+webkit+**mobile-safari**. **A3 regression:** api **135/135**, gateway **33/33**, flutter analyze 0 + **157/157** + build web ✓, SBOM 352 comp | `packages/shared/error-codes.json`, `apps/api/src/common/{config/throttle-config,error-codes.spec}.ts` + 9 services, `api_client.dart`, `error_code_mapping_test.dart`, `l10n/*.arb`, `package.json`, `.github/workflows/*`, `scripts/e2e-stack.sh` |
| 2026-07-23 | **✅ CI เขียวครบทุก job ที่ `2c535b1` (PR #1):** e2e **57 passed × 3 รอบติด** (Chrome+Mobile Chrome+WebKit) + api-gateway/integration/mobile/npm-audit/secret-scan/flutter-audit/sbom ผ่านหมด — เป็น commit แรกที่ CI เขียวจริงทั้งหมด (รอบ 6-8 ก่อนหน้าตกที่ lockout test; แก้ด้วย AUTH_LOCKED code). ผลตรงกับ SHA ที่ตรวจ | (ยืนยันบน GitHub Actions run 30018753306/30018753321) |
| 2026-07-23 | **แก้ 3 blockers จากผลตรวจ QA (GATE 1: FAIL → แก้ครบ):** **(1) Stable error codes** — backend แนบ `code`/`errorCode` ทุก user-facing error 48 จุด (scan per-item + HttpException ใน batches/packages/print-jobs; ข้อความไทยเดิมคงไว้ backward compat); mobile `serverErrorFromCode` map 24 codes → ARB srv* (th+en), กฎใหม่: code รู้จัก→ภาษา locale ผู้ใช้, ไม่มี code→ไทย passthrough เฉพาะ locale th / locale อื่น errGeneric — **จอ EN ไม่มีทางเห็นไทย**; scan item error ผ่านกฎเดียวกัน; i18n_test แทนเทส passthrough เดิม (QA ชี้ว่า encode พฤติกรรมผิด) ด้วย 3 เทสใหม่. **(2) Layout matrix ครบทุกหน้า Gate 1** — Login/Settings/รายการห่อ/สร้างห่อ/รายละเอียดห่อ/Scan/Print Jobs @ 320px/Pixel 7/Desktop × scale 1.0/1.3 × th/en = 84 เคส ด้วย fixture HttpClientAdapter (เรนเดอร์ข้อมูลจริงไม่ใช่ error state) + dump creator chain เมื่อพัง; **จับบั๊กจริง 6 จุดแล้วแก้หมด** (print jobs header, 2 sheets ไม่ scroll, StatusBadge, scan count row, lifecycle nodes, quantity row). **(3) Allowlist ระดับ class** — `classScope` + brace matching: settings_page ยกเว้นเฉพาะ `_PrinterSheet(+State)` ส่วนอื่นตรวจเต็ม (เปิด scope แล้วจับได้ทันที 2 string ที่เคยถูกซ่อน), ยกเว้นทั้งไฟล์เหลือเฉพาะที่ pattern = ทั้งไฟล์จริง + guard กัน allowlist ค้าง. ยืนยัน: **flutter test 152/152** · analyze 0 · build web ✓ · api 110/110 · tsc ✓ | `apps/api/src/modules/{scan,batches,packages,print-jobs}/*.service.ts`, `api_client.dart`, `models.dart`, `scan_page.dart`, `l10n/*.arb`, `test/{i18n,i18n_static_check,layout}_test.dart`, `print_jobs_page.dart`, `create_package_sheet.dart`, `domain_widgets.dart`, `package_detail_page.dart` |
